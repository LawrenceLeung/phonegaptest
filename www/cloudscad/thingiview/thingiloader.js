Thingiloader = function(event) {
  ...
  this.loadSTL = function(url) {
    ...
    workerFacadeMessage({'status':'message', 'content':'Downloading worker ' + url});  
    
	$.get(url, function(data) {
		  var reader = new BinaryReader(data);
		  workerFacadeMessage({'status':'message', 'content':'begin parse ' + url});  
		  
		  if (looksLikeBinary(reader)) {
		  me.loadSTLBinary(reader);
		  } else {
		  me.loadSTLString(data);
		  }
	},'text');    
  };

  this.loadOBJ = function(url) {
    workerFacadeMessage({'status':'message', 'content':'Downloading obj ' + url});  
    var file = this.load_binary_resource(url);
    this.loadOBJString(file);
  };

  this.loadJSON = function(url) {
    workerFacadeMessage({'status':'message', 'content':'Downloading json ' + url});
    var file = this.load_binary_resource(url);
  ...
  this.loadSTLString = function(STLString) {
    ...
    workerFacadeMessage({'status':'complete', 'content':me.ParseSTLString(STLString)});
  };

  this.loadSTLBinary = function(STLBinary) {
    workerFacadeMessage({'status':'message', 'content':'Parsing STL Binary...'});
    workerFacadeMessage({'status':'complete', 'content':me.ParseSTLBinary(STLBinary)});
  };

  this.loadOBJString = function(OBJString) {
    workerFacadeMessage({'status':'message', 'content':'Parsing OBJ String...'});
    workerFacadeMessage({'status':'complete', 'content':me.ParseOBJString(OBJString)});
  }
